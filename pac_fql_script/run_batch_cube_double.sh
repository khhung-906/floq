...
usage() {
  cat <<'EOF'
Usage:
  ./run_batch_cube_double.sh --alpha A [--offline_steps N] [--project_name PROJECT] [--critic_lr LR] [--actor_lr LR]
                            [--num_atoms N] [--v_min V] [--v_max V]

Example:
  ./run_batch_cube_double.sh --alpha 300 --project_name pac-fql-cube --offline_steps 1000000
EOF
}
