...
# PAC-FQL actor batch launcher for cube-double.
...
PROJECT_NAME="pac_fql_cube_triple"
CRITIC_LR="1e-4"
ACTOR_LR="5e-4"
NUM_ATOMS="301"
V_MIN="-300.0"
...
usage() {
  cat <<'EOF'
Usage:
  ./run_batch_cube_triple.sh --alpha A [--offline_steps N] [--project_name PROJECT] [--critic_lr LR] [--actor_lr LR]
                            [--num_atoms N] [--v_min V] [--v_max V]

Example:
  ./run_batch_cube_triple.sh --alpha 300 --project_name pac-fql-cube-triple --offline_steps 1000000
EOF
}
