...
class ImplicitQuantileValue(nn.Module):
    ...
    output_dim:int = 1
    encoder: nn.Module = None
    embed_quantile: bool = True  #whether to use cosine embedding for quantile
    quantile_embed_dim: int = 64 #how many dimensions to embed the quantile in
    ...
    def __call__(self, observations, actions=None, quantiles=None,):
        """Return values or critic values.
        ...
        if self.embed_quantile:
            quantiles = jnp.tile(quantiles, [1, self.quantile_embed_dim]) #(batch_size, return_embedding_dim)
            quantiles = (jnp.arange(1, self.quantile_embed_dim + 1, 1).astype(jnp.float32)* jnp.pi * quantiles)
            quantiles = jnp.cos(quantiles) #(batch_size, quantile_embed_dim)
        
        inputs.append(quantiles)
        inputs = jnp.concatenate(inputs, axis=-1)
        v = self.value_net(inputs)
        return v.squeeze(-1)
